...
static int sc_can_enable(const struct sc_can_cfg *config)
{
	 ...
	 */
	while (retries-- > 0) {
		sys_set_bits(config->reg_addr + SCCAN_ENR_OFFSET, SCCAN_CAN_ENABLE);
...
static int sc_can_disable(const struct sc_can_cfg *config)
{
	 ...
	 */
	while (retries-- > 0) {
		sys_clear_bits(config->reg_addr + SCCAN_ENR_OFFSET, SCCAN_CAN_ENABLE);
...
#define SCCAN_INIT(n)                                                                              \
	...
	static struct sc_can_data sc_can_data_##n = {                                              \
		...
		.tx_mutex = Z_MUTEX_INITIALIZER(sc_can_data_##n.tx_mutex),                         \
		.rx_mutex = Z_MUTEX_INITIALIZER(sc_can_data_##n.rx_mutex),                         \
