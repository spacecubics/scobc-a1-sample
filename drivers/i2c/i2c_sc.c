...
/* Registers */
#define SC_I2C_ENR_OFFSET     (0x0000) /* I2C Master Enable */
#define SC_I2C_TXFIFOR_OFFSET (0x0004) /* I2C Master TX FIFO */
#define SC_I2C_RXFIFOR_OFFSET (0x0008) /* I2C Master RX FIFO */
#define SC_I2C_BSR_OFFSET     (0x000C) /* I2C Master Bus Status */
#define SC_I2C_ISR_OFFSET     (0x0010) /* I2C Master Interrupt Status */
#define SC_I2C_IER_OFFSET     (0x0014) /* I2C Master Interrupt Enable */
#define SC_I2C_FIFOSR_OFFSET  (0x0018) /* I2C Master FIFO Status */
#define SC_I2C_FIFORR_OFFSET  (0x001C) /* I2C Master FIFO Reset */
#define SC_I2C_FTLSR_OFFSET   (0x0020) /* I2C Master FIFO Threshold Level Setting */
#define SC_I2C_SCLTSR_OFFSET  (0x0024) /* I2C Master SCL Timeout Setting */
#define SC_I2C_THDSTAR_OFFSET (0x0030) /* I2C Master START Hold Timing Setting */
#define SC_I2C_TSUSTOR_OFFSET (0x0034) /* I2C Master STOP Setup Timing Setting */
#define SC_I2C_TSUSTAR_OFFSET (0x0038) /* I2C Master Repeated START Setup Timing Setting */
#define SC_I2C_THIGHR_OFFSET  (0x003C) /* I2C Master Clock High Timing Setting */
#define SC_I2C_THDDATR_OFFSET (0x0040) /* I2C Master Data Hold Timing Setting */
#define SC_I2C_TSUDATR_OFFSET (0x0044) /* I2C Master Data Setup Timing Setting */
#define SC_I2C_TBUFR_OFFSET   (0x0048) /* I2C Master Bus Free Timing Setting */
#define SC_I2C_TBSMPLR_OFFSET (0x004C) /* I2C Master Bus Sampling Timing Setting */
#define SC_I2C_VER_OFFSET     (0xF000) /* I2C Master Controller IP Version */
...
#define SC_I2C_VER_MAJOR(x) (((x) & GENMASK(31, 24)) >> 24)
#define SC_I2C_VER_MINOR(x) (((x) & GENMASK(23, 16)) >> 16)
#define SC_I2C_VER_PATCH(x) (((x) & GENMASK(15, 0)) >> 0)
...
static int i2c_sc_init(const struct device *dev)
{
	...
	LOG_DBG("SC-OBC Module A1 I2C Master controller v%ld.%ld.%ld initialized", SC_I2C_VER_MAJOR(v),
		SC_I2C_VER_MINOR(v), SC_I2C_VER_PATCH(v));
...
static int i2c_sc_transfer(const struct device *dev, struct i2c_msg *msgs, uint8_t num_msgs,
			   uint16_t addr)
{
	const struct i2c_sc_cfg *config = dev->config;
	int ret = 0;
...
static DEVICE_API(i2c, i2c_sc_driver_api) = {
...
#define I2C_SC_INIT(n)                                                                             \
	...
	static struct i2c_sc_data i2c_sc_data_##n;                                                 \
                                                                                                   \
	I2C_DEVICE_DT_INST_DEFINE(n, i2c_sc_init, NULL, &i2c_sc_data_##n, &i2c_sc_cfg_##n,         \
				  POST_KERNEL, CONFIG_I2C_INIT_PRIORITY, &i2c_sc_driver_api);      \
	static void i2c_sc_##n##_irq_init(const struct device *dev)                                \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), 0, i2c_sc_isr, DEVICE_DT_INST_GET(n), 0);             \
                                                                                                   \
